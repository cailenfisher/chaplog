<script lang="ts">
	...
	const barColor = $derived(allDone ? 'bg-emerald-500' : noneStarted ? 'bg-red-400' : 'bg-amber-400');
	const statusText = $derived(
		total === 0 ? 'No tasks' : allDone ? 'All done! ✓' : `${done} of ${total} tasks`
	);
	const statusColor = $derived(
		total === 0 ? 'text-slate-400' : allDone ? 'text-emerald-600' : noneStarted ? 'text-red-500' : 'text-amber-600'
	);
	...
	const alerts = $derived(
		(() => {
			...
			for (const task of activeTasks) {
				...
				let streak = 0;
				for (const d of last7) { if (!doneSet.has(d)) streak++; else break; }
	...
	<div class="flex flex-col gap-6 lg:flex-row">
		...
		<div class="flex shrink-0 flex-col gap-4 lg:w-64">
			<div class="flex items-start gap-4">
				<div class="grid h-14 w-14 shrink-0 place-items-center rounded-full bg-indigo-100 text-xl font-bold text-indigo-700">
					{initials}
				</div>
				<div class="min-w-0 flex-1">
					<div class="flex items-center gap-1">
						...
						{#if onEdit}
							<button onclick={onEdit} class="ml-auto shrink-0 rounded-md p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600" title="Edit patient" aria-label="Edit patient">
						...
						{/if}
						{#if onDelete}
							<button onclick={confirmDelete} class="shrink-0 rounded-md p-1 text-slate-400 hover:bg-red-50 hover:text-red-500" title="Remove patient" aria-label="Remove patient">
								<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
			...
			<div class="space-y-1.5">
				<div class="flex justify-between text-xs text-slate-400">
					<span>Today's progress</span><span>{Math.round(pct)}%</span>
				</div>
			...
			<div class="flex flex-wrap gap-1.5">
				...
				{#if activeTasks.length === 0}
					<span class="text-xs italic text-slate-400">No tasks yet</span>
				{/if}
			</div>
		...
		<!-- Divider -->
		<div class="hidden self-stretch lg:block"><div class="h-full w-px bg-slate-100"></div></div>

		<!-- Right panel -->
		<div class="flex min-w-0 flex-1 flex-col gap-4">
			<div>
				<p class="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">7-day rolling completion · 30 days</p>
