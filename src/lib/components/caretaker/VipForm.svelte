<script lang="ts">
	...
	interface Props {
		vip?: VipSummary;
		onSave: (data: { display_name: string; activeThemeId: string }) => void | Promise<void>;
		onclose: () => void;
	}

	let { vip, onSave, onclose }: Props = $props();
	...
	async function save() {
		...
		if (!trimmed) return;
		await onSave({ display_name: trimmed, activeThemeId: themeId });
		onclose();
	}
...
>
	<form onsubmit={save} class="flex flex-col gap-6 p-6">
		<h2 class="text-xl font-bold text-slate-800">{isEdit ? 'Edit Patient' : 'Add Patient'}</h2>
