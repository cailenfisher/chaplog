<script lang="ts">
	...
	type TileState = 'pending' | 'confirming' | 'completed';
	let tileState = $state<TileState>('pending');
	let encouragement = $state('');

	// Grid placement driven by importance
	...
	// Responsive font sizes scaled to tile area
	...
	const confirmQuestionSize = $derived(
...
<!--
  The outer div is the grid item — its col/row span sets tile size.
  min-w-0 min-h-0 prevent grid blowout.
-->
<div class="{colClass} {rowClass} min-w-0 min-h-0" style="perspective: 1200px;">
	<div class="flip-inner relative w-full h-full" class:flipped={tileState === 'completed'}>
		<!-- ── FRONT FACE ────────────────────────────────────── -->
		<div
		...
		>
			{#if tileState === 'confirming'}
				<!-- Confirmation overlay — appears on top of the tile's own gradient -->
				<div
				...
				>
					<p
						...
						style="font-size: {confirmQuestionSize};"
					>
					...
					<div class="flex flex-col gap-3 w-full" style="max-width: min(22rem, 85%);">
						<button
							...
							style="padding: clamp(0.8rem, 2vh, 1.4rem);
			...
			{:else}
				<!-- Pending state — the entire tile is one big tap target -->
				<button
					onclick={tap}
				...
				>
					...
					{#if task.importance === 3}
						<span
							class="text-white/50 font-medium"
							style="font-size: clamp(0.8rem, 1.2vw, 1rem);"
						>
							Tap to complete
						</span>
					{/if}
		...
		>
			...
			<span
				class="text-emerald-400/60 font-medium"
...
<style>
	...
	/* Both faces must hide their backside to avoid showing through */
	.flip-face,
	.flip-back {
	...
	/* Back face starts rotated so it's hidden until the flip completes */
	.flip-back {
		transform: rotateY(180deg);
	}
</style>
