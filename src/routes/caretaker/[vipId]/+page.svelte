<script lang="ts">
	...
	async function callAction(actionName: string, fields: Record<string, string>) {
	...
	async function handleAddTask(formData: { name: string; icon: string; importance: 1 | 2 | 3 }) {
		await callAction('addTask', {
			name: formData.name,
			icon: formData.icon,
			importance: String(formData.importance)
		});
	}

	async function handleUpdateTask(formData: { name: string; icon: string; importance: 1 | 2 | 3 }) {
		await callAction('updateTask', {
	...
	async function softDelete(task: ReportTask) {
		if (confirm(`Remove "${task.name}"? Completion history will be preserved.`))
			await callAction('deleteTask', { taskId: task.id });
	}

	async function restore(task: ReportTask) {
		await callAction('restoreTask', { taskId: task.id });
	}
...
<svelte:head>
	<title>ChapLog — {vip.display_name} Report</title>
</svelte:head>
...
<div class="mb-5 flex w-fit gap-1 rounded-xl border border-slate-200 bg-white p-1 shadow-sm">
	{#each REPORT_TABS as { view, label }}
		<button
			...
			class="rounded-lg px-5 py-2 text-sm font-semibold transition-all
			       {activeView === view ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}"
		>
			{label}
		</button>
	{/each}
	...
	<button
		...
		class="rounded-lg px-5 py-2 text-sm font-semibold transition-all
		       {activeView === 'tasks' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'}"
	>
		Tasks
		<span class="ml-1.5 rounded-full px-1.5 py-0.5 text-xs {activeView === 'tasks' ? 'bg-white/20' : 'bg-slate-100 text-slate-500'}">
			{activeTasks.length}
		</span>
	</button>
</div>
...
{:else}
	...
	<div class="mb-4 flex items-center justify-between">
		...
		<button onclick={() => (showAddTask = true)} class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700 active:scale-95">
			+ Add task
		</button>
	</div>
			...
			{#each activeTasks as task (task.id)}
				<div class="flex items-center gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
					...
					<div class="flex shrink-0 items-center gap-1">
						...
						<button onclick={() => softDelete(task)} class="rounded-md border border-red-100 px-3 py-1.5 text-xs font-medium text-red-500 hover:bg-red-50">Remove</button>
					</div>
				</div>
			{/each}
					...
					<div class="space-y-2">
						{#each deletedTasks as task (task.id)}
							<div class="flex items-center gap-3 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-3 opacity-60">
								...
								<button onclick={() => restore(task)} class="shrink-0 rounded-md border border-indigo-200 px-3 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50">Restore</button>
							</div>
						{/each}
					</div>
				</div>
...
{#if showAddTask}
	<TaskForm onSave={async (d) => { await handleAddTask(d); showAddTask = false; }} onclose={() => (showAddTask = false)} />
{/if}

{#if editingTask}
	<TaskForm task={editingTask} onSave={async (d) => { await handleUpdateTask(d); editingTask = null; }} onclose={() => (editingTask = null)} />
{/if}
