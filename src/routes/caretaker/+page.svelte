<script lang="ts">
	...
	const allDoneCount = $derived(
		store.vips.filter((v) => {
			...
			return completionsForDate(v.completions, todayStr).filter((c) =>
...
<div class="mb-8 flex gap-4">
	...
	<div
		class="rounded-xl border px-5 py-4 shadow-sm
		       {allDoneCount === store.vips.length && store.vips.length > 0
...
<!-- Modals -->
{#if showAdd}
	<VipForm onclose={() => (showAdd = false)} />
{/if}

{#if editingVip}
	<VipForm vip={editingVip} onclose={() => (editingVip = null)} />
{/if}
